...
export class ChartService {
  ...
  getChartData(stock){
   ...
   this.service.getChart(stock)
   .subscribe(response  =>{
    this.chart.data = response["Time Series (5min)"];
    this.createArr(this.chart);
  ...
  createArr(chart){
    for (var each in this.chart.data){
      this.pair.name = new Date(each).getTime().toString();
      this.pair.value = this.chart.data[each]["4. close"];
      this.pairArr.push(this.pair);
     }
  ...
  //  }
   ...
   getStaticData() {
    this.getChartData('AAPL');
     console.log(this.pairArr);
     return [{"name": 'gege',"series":[{"name": 'hrehr',"value": 1.0}]}];
   }
}

export class NameValPair {
  'name': string
  'value': number
}
